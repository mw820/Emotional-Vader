...
emo_df = emo_df.drop_duplicates(ignore_index=True)
...
'''compute emo-score for every emotion'''
analyzer = SentimentIntensityAnalyzer()

for emotion in emotions:
    emo_df[emotion+'_emo_score']=emo_df['text'].apply(lambda x: analyzer.polarity_scores(x)[emotion])
    print(emotion)
...
for emotion in emotions:
    '''indices of posts which contain given emotion'''
    emotion_idx = np.array(emo_df[emo_df[emotion]==1].index)
    emotion_df = emo_df.iloc[emotion_idx,:]
    
   
    '''indices of all posts'''
    all_idx = np.array(emo_df.index) 
    
    '''indices of posts which doesn't  contain given emotion'''
    not_emotion_idx = np.array([e for e in all_idx if e not in emotion_idx])
    
    '''split idx which doesn't contain given emotion into training and validation set ~ 50/50.
       Creatin corrensponding dataframes
       Counting mean and std'''
    ...
    mean_and_std_train[emotion] = np.array([np.mean(train_df[emotion+'_emo_score']), np.std(train_df[emotion+'_emo_score'])])
    ...
    mean_and_std_validation[emotion] = np.array([np.mean(train_df[emotion+'_emo_score']), np.std(train_df[emotion+'_emo_score'])])
    
    '''Computing z-score for emo_train_df (using mean and std from validation set)'''
    emo_train_df = pd.concat([emotion_df, train_df],axis=0,ignore_index=True)
    
    mean_v = mean_and_std_validation[emotion][0]
    std_v = mean_and_std_validation[emotion][1]
    z_score_v = (emo_train_df['text'].apply(lambda x: (analyzer.polarity_scores(x)[emotion]-mean_v)/std_v)).sort_values()
    
    '''creating subset of emo_train_df with particular emotion and its z-score'''
    emo_t = emo_train_df[['text', emotion, emotion+'_emo_score']].copy()
    emo_t[emotion + '_z_score'] = z_score_v
    dfs_train[emotion]=[]
    dfs_train[emotion].append(emo_t)
    
    print(emotion)
    
    '''''''''''''''''''''''''''''''''''  2nd fold   '''''''''''''''''''''''''''''''''''''''''''''''''''
    
    '''Computing z-score for emo_validation_df (using mean and std from training set)'''
    emo_validation_df = pd.concat([emotion_df, validation_df],axis=0,ignore_index=True)

    mean_t = mean_and_std_train[emotion][0]
    std_t = mean_and_std_train[emotion][1]
    z_score_t = (emo_validation_df['text'].apply(lambda x: (analyzer.polarity_scores(x)[emotion]-mean_t)/std_t)).sort_values()
    emo_validation_df[emotion + '_z_score_t'] = z_score_t
    
    '''creating subset of emo_validation_df with particular emotion and its z-score'''
    emo_v = emo_validation_df[['text', emotion, emotion+'_emo_score']].copy()
    emo_v[emotion + '_z_score'] = z_score_v
    dfs_validation[emotion]=[]
    dfs_validation[emotion].append(emo_v)
...
'''Precision, recall, accuracy for all the emotion.'''

def performance(zscore,emotion,dfs):
    df = dfs[emotion][0]
    TP = len(df[(df[emotion+'_z_score']>zscore) & (df[emotion]==1)])
    TN = len(df[(df[emotion+'_z_score']<=zscore) & (df[emotion]==0)])
    FP = len(df[(df[emotion+'_z_score']>zscore) & (df[emotion]==0)])
    FN = len(df[(df[emotion+'_z_score']<=zscore) & (df[emotion]==1)])
...
# for 1.96 (0.025 upper tailed test)

results = []
for emotion in emotions:
    a, p, r, f1 = performance(1.96,emotion,dfs_train)
    results.append((a,p,r,f1))
    
aa=pd.DataFrame(results, columns = ['Accuracy', 'Precision', 'Recall', 'F1'], index=[e for e in emotions])

# for 1.645 (0.05 upper tailed test)
results = []

for emotion in emotions:
    a, p, r, f1 = performance(1.645,emotion,dfs_train)
    results.append((a,p,r,f1))
    
bb=pd.DataFrame(results, columns = ['Accuracy', 'Precision', 'Recall', 'F1'], index=[e for e in emotions])

pd.concat([aa, bb], axis=1, keys=['zscore: 1.96', 'zscore: 1.645'])
...
for emotion in emotions:
    ...
    ax[0,0].set_xlabel('emo-vader score', fontsize =13)
    ax[0,0].set_ylabel('Frequency', fontsize =13)
    ax[0,0].set_title(emotion+': Histogram of all emo-vader scores', fontsize = 15)
    ax[0,0].hist(np.array(dfs_train[emotion][0][emotion+'_emo_score']),bins='auto')[2]


    ax[0,1].set_xlabel('z-score', fontsize =13)
    ax[0,1].set_ylabel('Frequency', fontsize =13)
    ax[0,1].set_title(emotion+': Histogram of all z-scores', fontsize =15)
    ax[0,1].hist(np.array(dfs_train[emotion][0][emotion+'_z_score']),bins='auto')[2]

    df = dfs_train[emotion][0]
    ax[1,0].set_xlabel('z-score', fontsize =13)
    ax[1,0].set_ylabel('Frequency', fontsize =13)
    ax[1,0].set_title(emotion+': Histogram of all z-scores for post elicting emotion', fontsize =15)
    ax[1,0].hist(np.array(df[df[emotion]==1][emotion+'_z_score']),bins='auto')[2]

    ax[1,1].set_xlabel('z-score', fontsize =13)
    ax[1,1].set_ylabel('Frequency',fontsize =13)
    ax[1,1].set_title(emotion+': Histogram of all z-scores for post NOT elicting emotion', fontsize =15)
    ax[1,1].hist(np.array(df[df[emotion]==0][emotion+'_z_score']),bins='auto')[2]
...
# histogram and distribution of scores for comutet zscores in 2nd step. 1.75 (10% significance level) itp.
# Prcision, recall, accuracy for all the emotion.
# plot all the z-scores i got
# plot all scores computed by emo-vader
# plot emo-vader scores and z-scores for posts that do not elict an emotion and plot scorest that elict an emotion.
# fix a-posteriori
