...
# # adding averaged scores for words from vader which are not present in any of 8 files 

# In[3]:


vader_lexicon = list(pd.read_csv("vader_lexicon.txt", delimiter = "\t", header=None)[0])

emotions = ['anger','disgust','fear','joy','sadness','surprise','trust','anticipation']

emo_words = []
for emotion in emotions:
    df = pd.read_csv(emotion+"-scores.txt", delimiter = "\t", header=None)
    'can add the same word ut to 8 times but it makes no difference'
    emo_words.extend(list(df.iloc[:,0]))

lacking_words = []
for word in vader_lexicon:
    if word not in emo_words:
        lacking_words.append(word)
...
for emotion in emotions:
    syns[emotion] = dict()
    df = pd.read_csv(emotion+"-scores.txt", delimiter = "\t", header=None)
    for word in lacking_words:
        for synset in wn.synsets(word):           
            for lemma in synset.lemmas():
                '''here I could add another condition not to add extra lemma_name if it alerady exist in syns[emotion][word]
                   It may save computational power but it does't make a difference at the end and
                   when I code the code it it looks really ugly'''
                if lemma.name() in list(df[0]) and word in syns[emotion]:   
#                     syns[emotion][word].append([lemma.name(), float(df.iloc[:,1][df.iloc[:,0]==lemma.name()])])
                    syns[emotion][word].append(float(df.iloc[:,1][df.iloc[:,0]==lemma.name()]))
                elif lemma.name() in list(df[0]):
                    syns[emotion][word] = []
#                     syns[emotion][word].append([lemma.name(), float(df.iloc[:,1][df.iloc[:,0]==lemma.name()])])
                    syns[emotion][word].append(float(df.iloc[:,1][df.iloc[:,0]==lemma.name()]))
        if word in syns[emotion]:           
            syns[emotion][word] = np.mean(syns[emotion][word])
    
    
    df_new_words = pd.DataFrame(syns[emotion].items())
    df = df.append(df_new_words)
    df.to_csv (emotion+r'-updated.txt', index = False, header=False, sep='\t')
...
## example - updated lexicon works... funny. It is because wordnet find word "love" as synonym of "fucked"

sentence = "You are fucked"
...
for synset in wn.synsets('fucked'):
    for lemma in synset.lemmas():
